...
def check():
    ...
    with open(kb_file, 'r', encoding='utf-8') as kf:
        ...
        for line in reader:
            ...
            try:
                driver4.get(build_url)

            except:
                print(kb, 'のビルド番号取得ページが存在しません。')

            try:
                update_kb = driver4.find_element_by_css_selector('#supLeftNav > div.supLeftNavCategory.supLeftNavActiveCategory > ul > li:nth-child(2) > a')
            except NoSuchElementException:
                print(kb, '要素が見つかりません。更新情報がない可能性があります')
                continue

            update_kb_text = update_kb.text
            for i_index_kb in range(len(update_kb_text.split(' '))):
                is_kb_item = update_kb_text.split(' ')[i_index_kb]
                if re.match('^.*KB[0-9]{7}.*$', is_kb_item):
                    kb_item = is_kb_item.split('KB')[1][:7]


                    if kb_item < kb:
                        print(kb, '抽出されたKB番号が対象のKB番号より小さいため更新はしません')
                    elif kb_item != kb:
                        if kb_item not in after_kb_num_list:
                            before_kb_num_list.append(kb)
                            after_kb_num_list.append(kb_item)


            driver4.close()
        
        print('Before: [',end='')
        print(*before_kb_num_list, sep=',' ,end='')
        print(']')
        print('After : [',end='')
        print(*after_kb_num_list, sep=',' ,end='')
        print(']')

        if not after_kb_num_list:
            print('更新情報はありません')
            return
    ...
    with open(all_file, 'a', encoding='utf-16', newline='') as wf:
            ...
            with open(small_file, 'a', encoding='utf-16', newline='') as wsf:
                ...
                # KB番号リストより順に値を取得
                for i_update_kb in range(len(after_kb_num_list)):
                    kb = after_kb_num_list[i_update_kb]
                            ...
                            if (pre_kb != kb) or (multiple_build == 1 and add_info4 != pre_add_info4):
                                ...
                                while True:
                                    try:
                                        ...
                                        print(kb, 'NoSuchElementException: build_url再取得中')
                                        burl_times += 1
                                        if burl_times == 3:
                                ...
                                if "." in element_build_text:
                                    ...
                                    if ("および" in element_build_text) or ("and" in element_build_text):
                                        ...
                                        build = element_build.text.split("(OS ビルド ")[1]
                                        add_info5 = build.split('.')[0]
                                        add_info6 = build.split('.')[1].split(')')[0]
                                    elif "（OS ビルド " in element_build_text:               # 全角()あり(日本語)
                                        build = element_build.text.split("（OS ビルド ")[1]
                                        add_info5 = build.split('.')[0]
                                        add_info6 = build.split('.')[1].split('）')[0]
                                    elif "(OS Build " in element_build_text:                # ()あり(英語)
                                        build = element_build.text.split("(OS Build ")[1]
                                        add_info5 = build.split('.')[0]
                                        add_info6 = build.split('.')[1].split(')')[0]
                                    elif "ビルド " in element_build_text:
                                        build = element_build.text.split("ビルド ")[1]
                                        add_info5 = build.split('.')[0]
